...
//  success = HalSensorReadReg(TMP006_REG_ADDR_VOLTAGE, (uint8 *)&v,IRTEMP_REG_LEN );
  success = HalSensorReadReg(TMP102_REG_ADDR_TEMPERATURE, (uint8 *)&t,IRTEMP_REG_LEN );	//Added by Parth
...
static void HalIRTempSelect(void)
{
  ...
  HalI2CInit(TMP102_I2C_ADDRESS, i2cClock_267KHZ);	//Added by Parth
}
